...
class TBXC(nn.Module):
    def __init__(self, args, config, device):
        ...
        self.lstm_fc = nn.Linear(config.embed_size * 2, config.embed_size, bias=False)
        if args.interaction_mode == 'mlp':
            self.gelu = nn.GELU()
            self.weibo_linear = nn.Linear(1,
                                          config.comment_max_seq_length,
                                          bias=False)
            self.comment_linear = nn.Linear(config.comment_max_seq_length,
                                            config.comment_max_seq_length,
                                            bias=False)
            self.bias = nn.Parameter(torch.zeros(config.comment_max_seq_length, ))
    ...
    def forward(self, input_weibos, input_comments, input_labels, mode='train'):
        ...
        # shape: (batch_size, comment_max_length, hidden_size * 2 = embed_size)
        comment_lstm_out, _ = self.lstm(comment_hidden_state)
        # shape: (batch_size, comment_max_length, embed_size)
        # comment_lstm_out = self.lstm_fc(comment_lstm_out)
        ...
        if self.args.interaction_mode == 'dot' or self.args.interaction_mode == 'minus_dot':
            ...
            if self.args.interaction_mode == 'dot':
                interaction_hidden_state = torch.mul(weibo_sentence_embedding, comment_lstm_out)
            else:
                interaction_hidden_state = -torch.mul(weibo_sentence_embedding, comment_lstm_out)
            ...
            # shape: (batch_size, comment_max_length, embed_size)
            interaction_hidden_state = torch.sub(comment_lstm_out, weibo_sentence_embedding)
            interaction_hidden_state = self.dropout(interaction_hidden_state)
            ...
            weibo_sentence_embedding = weibo_sentence_embedding.permute(0, 2, 1)
            # shape: (batch_size, embed_size, comment_max_length)
            comment_lstm_out = comment_lstm_out.permute(0, 2, 1)

            # shape: (batch_size, comment_max_length, embed_size)
            interaction_hidden_state = self.gelu(self.weibo_linear(weibo_sentence_embedding) +
                                                 self.comment_linear(comment_lstm_out) +
                                                 self.bias).permute(0, 2, 1)
            interaction_hidden_state = self.dropout(interaction_hidden_state)
            ...
            interaction_hidden_state = torch.cat((comment_lstm_out, weibo_sentence_embedding), 2)
            # shape: (batch_size, comment_max_length, embed_size)
