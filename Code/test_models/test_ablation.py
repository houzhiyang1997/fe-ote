...
def evaluate(model, batch_count, batch_X, batch_y, device, mode, id2tag):
    ...
    with torch.no_grad():
        for i in range(batch_count):
            ...
            labels = batch_y[i]
            tensor_labels = torch.LongTensor(labels).to(device)

            # shape: (batch_size, seq_length)
            logits = model(input_weibos, input_comments)

            loss = F.cross_entropy(logits.permute(0, 2, 1), tensor_labels)

            loss_total += loss.data

            pred = logits.argmax(axis=2)
            pred_tags = convert_id2tag(pred.cpu().numpy().tolist(), id2tag)
...
def train(args, device):
    config = ablation.Config(args, num_outputs=3)
    ...
    model = ablation.Ablation(args, config, device).to(device)
...
parser.add_argument('-a',
                    '--ablation',
                    help='wce: w/ weibo, w/o CRF , '
                         'ce: w/o weibo, w/o CRF',
                    default='wce')
