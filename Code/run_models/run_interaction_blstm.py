...
def evaluate(model, data_iter, device, mode, id2tag):
    ...
    with torch.no_grad():
        for weibos, comments, masks, labels in data_iter:
...
def train(args, device):
    ...
    for epoch in range(config.num_epochs):
        ...
        for weibos, comments, masks, labels in train_iter:
            optimizer.zero_grad()
            loss = model(weibos, comments, labels, mode='train', input_masks=masks)
