...
def trunc_and_padding(texts, config, vocab, tokenizer):
    ...
    weibo_indices, comments_indices, masks = [], [], []
    for text in input_weibos:
        words = list(text)
        ids = [vocab['PAD']] * config.weibo_max_seq_length
        if len(words) > config.weibo_max_seq_length:
            words = words[:config.weibo_max_seq_length]
        i = 0
        for word in words:
    ...
    for text in input_comments:
        ...
        comments_indices.append(tokens.input_ids)
        masks.append(tokens.attention_mask)

    return torch.LongTensor(weibo_indices), torch.LongTensor(comments_indices), torch.LongTensor(masks)
...
def evaluate(model, data_iter, device, mode, id2tag):
    ...
    with torch.no_grad():
        for input_weibos, input_comments, input_masks, input_labels in data_iter:

            # shape: (batch_size, seq_length)
            loss, pred = model(input_weibos,
                               input_comments,
                               input_labels,
                               mode,
...
def train(args, device):
    config = interaction.Config(args, num_outputs=3)
    ...
    train_weibos, train_comments, train_masks = trunc_and_padding(train_inputs,
                                                                  config,
                                                                  vocab,
                                                                  tokenizer)
    dev_weibos, dev_comments, dev_masks = trunc_and_padding(dev_inputs,
                                                            config,
                                                            vocab,
                                                            tokenizer)
    test_weibos, test_comments, test_masks = trunc_and_padding(test_inputs,
                                                               config,
                                                               vocab,
                                                               tokenizer)
    ...
    train_dataset = Data.TensorDataset(train_weibos, train_comments, train_masks, train_labels)
    dev_dataset = Data.TensorDataset(dev_weibos, dev_comments, dev_masks, dev_labels)
    test_dataset = Data.TensorDataset(test_weibos, test_comments, test_masks, test_labels)
    ...
    model = interaction.Interaction(args, config, device, embed).to(device)
    ...
    for epoch in range(config.num_epochs):
        ...
        for input_weibos, input_comments, input_masks, input_labels in train_iter:
            optimizer.zero_grad()
            loss = model(input_weibos, input_comments, input_labels, mode='train',
                         input_masks=input_masks)
